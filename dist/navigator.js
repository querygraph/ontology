...
export const topicLevels = ["area", "focus", "topic"];
export const topicContentKinds = ["all", "talk", "person", "event", "project", "company", "video", "photo"];
export const topicContentSorts = ["newest", "relevance", "strength", "name"];
...
export function topicChoiceBacktrackContext(steps, next) {
    let choiceIndex = -1;
    for (let index = steps.length - 1; index >= 0; index -= 1) {
        if (topicChoiceActions.has(steps[index].action)) {
            choiceIndex = index;
            break;
        }
    }
    if (choiceIndex < 0 || steps.slice(choiceIndex + 1).some((step) => step.action === "backtrack"))
        return null;
    const previous = steps[choiceIndex];
    if (!previous.level || !previous.toId)
        return null;
...
const topicSearchIndexCache = new WeakMap();
export function topicConceptMap(snapshot) {
    return new Map(snapshot.concepts.map((concept) => [concept.id, concept]));
}
export function topicPrimaryTrail(snapshot, activeId) {
    ...
    if (!trail.some((concept) => concept.id === active.id))
        trail.push(active);
...
export function topicPrimaryRoute(snapshot, activeId) {
    const route = {};
    for (const concept of topicPrimaryTrail(snapshot, activeId)) {
        if (concept.level === "area")
            route.areaId = concept.id;
        else if (concept.level === "focus")
            route.focusId = concept.id;
        else
            route.topicId = concept.id;
    }
    return route;
}
/** Resolve a URL or caller-provided route without allowing nonexistent graph edges. */
export function topicNavigatorRoute(snapshot, references = {}) {
    ...
    if (topic) {
        if (focus && topic.parentIds.includes(focus.id)) {
            ...
            if (focusRoute.areaId)
                return { areaId: focusRoute.areaId, focusId: focus.id, topicId: topic.id };
        }
        return topicPrimaryRoute(snapshot, topic.id);
    }
    if (focus) {
        if (area && focus.parentIds.includes(area.id))
            return { areaId: area.id, focusId: focus.id };
...
export function topicRouteAfterChoice(snapshot, current, concept) {
    const route = topicNavigatorRoute(snapshot, current);
    if (concept.level === "area")
        return { areaId: concept.id };
...
export function topicRouteFromSearchParams(snapshot, search) {
...
export function topicRouteSearchParams(snapshot, route, search = "") {
...
export function topicNavigatorBands(snapshot, active = null) {
    const byId = topicConceptMap(snapshot);
...
export function topicBandActiveIds(snapshot, active = null) {
    const route = typeof active === "string" ? topicPrimaryRoute(snapshot, active) : topicNavigatorRoute(snapshot, active || {});
    return { area: route.areaId, focus: route.focusId, topic: route.topicId };
}
export function topicPathLabel(snapshot, concept) {
...
export function rememberTopicSearchSubmission(seen, cacheKey) {
    if (seen.has(cacheKey) || seen.size >= 49)
        return false;
    seen.add(cacheKey);
    return true;
}
export function localTopicSearch(snapshot, query, limit = 12) {
    const needle = normalizeTopicLabel(query);
    if (!needle)
        return { results: [], canSuggest: false };
...
function topicSearchIndex(snapshot) {
...
export function parseTopicSearchPayload(value, snapshot, query) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return localTopicSearch(snapshot, query);
    const record = value;
...
export function normalizeTopicContentResult(value, fallback) {
...
export function topicContentDate(item) {
...
function orderedChildren(snapshot, parent, level) {
    const byId = topicConceptMap(snapshot);
...
function topicReference(snapshot, reference, level) {
    if (!reference)
        return undefined;
    return snapshot.concepts.find((concept) => concept.level === level && (concept.id === reference || concept.slug === reference));
}
function topicEditDistance(left, right) {
    if (Math.abs(left.length - right.length) > typoThreshold(right))
...
function typoThreshold(value) {
    return value.length >= 8 ? 2 : value.length >= 4 ? 1 : 0;
}
function parseTopicContentItem(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return [];
    const item = value;
    ...
    if (!id || !name || !isTopicContentKind(kind) || kind === "all")
        return [];
    return [{
            id, kind, name,
...
function parseContentCounts(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return null;
    const counts = {};
    const aliases = {
    ...
    for (const [key, raw] of Object.entries(value)) {
        ...
        if (normalized)
            counts[normalized] = finiteNonnegative(raw) ?? 0;
    }
    return counts;
}
...
function isTopicContentKind(value) {
    return Boolean(value && topicContentKinds.includes(value));
}
function isTopicContentSort(value) {
    return Boolean(value && topicContentSorts.includes(value));
}
